...
class ChristmasPastryShopApp:
    ...
    VALID_TYPR_DELICACIES = {"Gingerbread": Gingerbread, "Stolen": Stolen}
    ...
    def add_delicacy(self, type_delicacy: str, name: str, price: float):
        if type_delicacy not in self.VALID_TYPR_DELICACIES:
            raise Exception(f"{type_delicacy} is not on our delicacy menu!")
        name_delicacy = self._find_name_delicacy(name)
        if name_delicacy:
            raise Exception(f"{name} already exists!")
        new_delicacy = self.VALID_TYPR_DELICACIES[type_delicacy](name, price)
    ...
    def reserve_booth(self, number_of_people: int):
        ...
        num = booth.booth_number
        return f"Booth {num} has been reserved for {number_of_people} people."
