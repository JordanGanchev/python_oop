...
class SummitQuestManagerApp:
    TYPE_CLIMBER = {'ArcticClimber': ArcticClimber, 'SummitClimber': SummitClimber}
    TYPE_PEAK = {'ArcticPeak': ArcticPeak, 'SummitPeak': SummitPeak}
    ...
    def register_climber(self, climber_type: str, climber_name: str):
        if climber_type not in self.TYPE_CLIMBER:
            return f"{climber_type} doesn't exist in our register."
        climber = self._get_climber(climber_name)
        if climber is not None:
            return f"{climber_name} has been already registered."
        new_climber = self.TYPE_CLIMBER[climber_type](climber_name)
        self.climbers.append(new_climber)
        return f"{climber_name} is successfully registered as a {climber_type}."

    def peak_wish_list(self, peak_type: str, peak_name: str, peak_elevation: int):
        if peak_type not in self.TYPE_PEAK:
            return f"{peak_type} is an unknown type of peak."
        new_peak = self.TYPE_PEAK[peak_type](peak_name, peak_elevation)
